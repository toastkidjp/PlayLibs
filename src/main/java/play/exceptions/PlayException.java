...
public abstract class PlayException extends RuntimeException {
    ...
    public PlayException(final String message) {
        super(message);
        setId();
    }

    public PlayException(final String message, final Throwable cause) {
        super(message, cause);
        setId();
    }

    void setId() {
        final long nid = atomicLong.incrementAndGet();
    ...
    public static StackTraceElement getInterestingStrackTraceElement(final Throwable cause) {
        for (final StackTraceElement stackTraceElement : cause.getStackTrace()) {
            if (stackTraceElement.getLineNumber() > 0
                    /*&& Play.classes.hasClass(stackTraceElement.getClassName())*/) {
