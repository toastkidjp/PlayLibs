...
import play.libs.ws.F.EventStream;
import play.libs.ws.F.Option;
...
public class Http {
    ...
    public static class Request implements Serializable {
         ...
         */
        public String encoding = WS.DEFAULT_WEB_ENCODING;
        /**
        ...
        public static Request createRequest(
            ...
            if( _contentType == null ) {
                ...
                final ContentTypeWithEncoding contentTypeEncoding = parseContentType( _contentType );
        ...
        protected void parseXForwarded() {
            ...
            if (/*Play.configuration.containsKey("XForwardedSupport") &&*/ headers.get("x-forwarded-for") != null) {
                if (/*!("all".equals(Play.configuration.getProperty("XForwardedSupport", "127.0.0.1")))
                        &&*/ !Arrays.asList("127.0.0.1"/*Play.configuration.getProperty("XForwardedSupport", "127.0.0.1").split("[\\s,]+")*/).contains(remoteAddress)
                        ) {
        ...
        protected void authorizationInit() {
            ...
            if (header != null && header.value().startsWith("Basic ")) {
                ...
                try {
                    decodedData = new String(Codec.decodeBASE64(data)).split(":");
        ...
        public boolean isModified(final String etag, final long last) {
            if (!(headers.containsKey("if-none-match") && headers.containsKey("if-modified-since"))) {
                ...
                if (!browserEtag.equals(etag)) {
                    ...
                    try {
                        final Date browserDate = getHttpDateFormatter().parse(headers.get("if-modified-since").value());
    ...
    public static class Response {
        ...
        public void cacheFor(final String etag, final String duration, final long lastModified) {
            final int maxAge = Time.parseDuration(duration);
            setHeader("Cache-Control", "max-age=" + maxAge);
            setHeader("Last-Modified", getHttpDateFormatter().format(new Date(lastModified)));
            setHeader("Etag", etag);
        }
    ...
    public static class ContentTypeWithEncoding {
        ...
        public ContentTypeWithEncoding(final String contentType, final String encoding) {
            this.contentType = contentType;
            this.encoding = encoding;
        }
    }

    public static ContentTypeWithEncoding parseContentType( final String contentType ) {
        if( contentType == null ) {
            return new ContentTypeWithEncoding("text/html".intern(), null);
        } else {
            final String[] contentTypeParts = contentType.split(";");
            final String _contentType = contentTypeParts[0].trim().toLowerCase();
            String _encoding = null;
            // check for encoding-info
            if( contentTypeParts.length >= 2 ) {
                final String[] encodingInfoParts = contentTypeParts[1].split(("="));
    ...
    private static Map<String, String> initLower2UppercaseHttpHeaders() {
        ...
        final String path = "/play/libs/ws/http_headers.properties";
        final InputStream in = Http.class.getResourceAsStream(path);
        if (in == null) {
            throw new RuntimeException("Error reading " + path);
        }
        final List<String> lines = IO.readLines( in );
     ...
     */
    public static String fixCaseForHttpHeader( final String headerName) {
        if (headerName == null) {
            return null;
        }
        final String correctCase = lower2UppercaseHttpHeaders.get(headerName.toLowerCase());
        if ( correctCase != null) {
            return correctCase;
        }
