...
public class Time {
    ...
    public static int parseDuration(final String duration) {
        ...
        if (days.matcher(duration).matches()) {
            final Matcher matcher = days.matcher(duration);
            matcher.matches();
            toAdd = Integer.parseInt(matcher.group(1)) * (int) TimeUnit.DAYS.toSeconds(1);
        } else if (hours.matcher(duration).matches()) {
            final Matcher matcher = hours.matcher(duration);
            matcher.matches();
            toAdd = Integer.parseInt(matcher.group(1)) * (int) TimeUnit.HOURS.toSeconds(1);
        } else if (minutes.matcher(duration).matches()) {
            final Matcher matcher = minutes.matcher(duration);
            matcher.matches();
            toAdd = Integer.parseInt(matcher.group(1)) * (int) TimeUnit.MINUTES.toSeconds(1);
        } else if (seconds.matcher(duration).matches()) {
            final Matcher matcher = seconds.matcher(duration);
            matcher.matches();
     ...
     */
    public static Date parseCRONExpression(final String cron) {
        try {
     ...
     */
    public static long cronInterval(final String cron) {
     ...
     * with an invalid interval between
     */
    public static long cronInterval(final String cron, final Date date) {
        try {
    ...
    public static class CronExpression implements Serializable, Cloneable {
        ...
        public Date getNextInvalidTimeAfter(final Date date) {
            ...
            final Calendar adjustCal = Calendar.getInstance();
         ...
         */
        public long getNextInterval(final Date date) {
            final Date nextValid = getNextValidTimeAfter(date);
            final Date nextInvalid = getNextInvalidTimeAfter(nextValid);
            final Date nextNextValid = getNextValidTimeAfter(nextInvalid);
        ...
        /**
         * Cron式をパースして返す.
         * ただし、通常のCron式と異なるので注意.
         * 秒 分 時 日 月 ?
         *
         * @author Toast kid
         *
         */
        public static Date parseCron(final String cron) {
            if (cron == null || "".equals(cron) || "never".equalsIgnoreCase(cron)) {
                System.err.printf("Skipping job %s, cron expression is not defined");
                return null;
            }
            try {
                final Date now = new Date();
                final CronExpression cronExp = new CronExpression(cron);
                final Date nextDate = cronExp.getNextValidTimeAfter(now);
                if (nextDate == null) {
                    System.err.printf(
        ...
        /**
         * Cron式をパースし、unixtime[ms] で返す.
         * @param cron Cron式
         * @return cron式をパースした結果のunixtime[ms], cron式が不正な場合は0l.
         */
        public static long cronToMillis(final String cron) {
            try {
                return parseCron(cron).getTime();
            } catch (final Exception e) {
                return 0l;
            }
        }
